...
namespace Assignment6
{
    public class OrderService
    {
        ...
        public void AddOrder(Order order)
        {
            ...
            {
                throw new Exception(string.Format($"订单号为{order.OrderId}的订单已存在"));
            }
            orders.Add(order);
        }
        ...
        public void RemoveOrder(int orderId)
        {
            ...
            if (order == null)
            {
                throw new Exception(string.Format($"订单号为{orderId}的订单不存在"));
            }
            orders.Remove(order);
        ...
        public void ModifyOrder(Order order)
        {
            ...
            {
                throw new Exception(string.Format($"订单号为{order.OrderId}的订单不存在")) ;
            }
            orders.Remove(existingOrder);
            Console.WriteLine($"订单号为{order.OrderId}的订单修改成功");
            orders.Add(order);
        }
        ...
        {
            return orders.Where(o => o.OrderCustomer.Name == customerName).ToList();
        }
        ...
        {
            return orders.Where(o => o.Details.Any(d => d.Product.ProductName == productName)).ToList();
        }
